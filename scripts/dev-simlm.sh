#!/usr/bin/env bash
# Preflight + run the simlm dev stack.
#
# Chain:
...
if ! docker info > /dev/null 2>&1; then
  echo "Docker daemon not reachable. Start Docker Desktop (or colima) and re-run pnpm dev:simlm." >&2
  exit 1
fi
...
# `container_name: simlm-postgres` so the second project errors with a
...
if [ "$count" = "0" ]; then
  printf "simlm pairs table is empty. Seed now (vi + chatterbot)? (y/N) "
  read -r answer
  case "$answer" in
