...
├─ vite.config.ts          # port 5174, proxy /api → http://localhost:3010 (portf api instance), @tanstack/router-plugin + @mdx-js/rollup wired in
├─ tsconfig.json
├─ vitest.config.ts
...
│  ├─ routes/              # TanStack Router file-based: __root.tsx (PortfShell), index.tsx, chat.$threadId.tsx, chat.$threadId.artifact.$kind.$slug.tsx, artifact.$kind.$slug.tsx, __404.tsx — see §5
...
### 5.2 Route tree (TanStack Router, file-based)

**Router choice:** `@tanstack/react-router`, not `react-router`. Rationale:

- The route tree literally lives in the filesystem (`routes/chat.$threadId.artifact.$kind.$slug.tsx`) — the never-unmount discipline is a filesystem invariant, not a runtime-enforced rule a future contributor could accidentally break by moving a `<Route>` element.
- Path params and search params are end-to-end typed (codegen reads the filenames). No `useParams<{...}>()` assertions; no manual `URLSearchParams` parsing for `?back=`.
- First-class TanStack Query integration (already in apps/portf). Route loaders can prefetch query data and hydrate immediately.
- Trade-off: diverges from `apps/web`'s `react-router 7`. Cost is acceptable because the two apps are isolated.

**File layout** (under `apps/portf/src/routes/`):

```
routes/
├─ __root.tsx                                   PortfShell — wraps every route, never unmounts
├─ index.tsx                                    /             → HomePane (V2 spotlight)
├─ chat.tsx                                     /chat/*       layout wrapper (redirect-only at /chat exact)
├─ chat.$threadId.tsx                           /chat/:id     ChatPane — owns its inner <Outlet />
├─ chat.$threadId.artifact.$kind.$slug.tsx      /chat/:id/artifact/:kind/:slug  ArtifactPane (in nested Outlet)
├─ artifact.$kind.$slug.tsx                     /artifact/:kind/:slug  ArtifactStandalone (deep link)
└─ __404.tsx                                    catch-all NotFound
```

The `routeTree.gen.ts` file is auto-generated by `@tanstack/router-plugin` at Vite dev/build time. Added to `.gitignore` AND `tsconfig.json`'s `include` so type-check sees it but git ignores it.

**Resulting URL → DOM shape table:**

| URL | `__root` (PortfShell) renders | Notes |
|---|---|---|
| `/` | Sidebar (empty state) · `<HomePane>` | V2 spotlight, no thread |
| `/chat` | redirect to `/chat/<freshUuid>` | TanStack Router's `beforeLoad` on `chat.tsx` issues a `throw redirect({ to: '/chat/$threadId', params: { threadId: uuid() } })` |
| `/chat/:id` | Sidebar · `<ChatPane>` (full width) | V1 conversation, empty nested outlet |
| `/chat/:id/artifact/:kind/:slug` | Sidebar · `<ChatPane>` (left 46%) · `<ArtifactPane>` (right 54%, slides in via CSS transition) | Both panes mounted; ChatPane's `<Outlet />` paints the artifact child |
| `/artifact/:kind/:slug?back=…` | Sidebar · `<ArtifactStandalone>` (full width) | Deep link, no thread context. `?back` is `validateSearch`'d via valibot. Mobile ← BACK navigates to `search.back ?? '/'` |
| any other path | Sidebar · `<__404>` | Sidebar still rendered so user can pick a thread |

**Why this works:**

- `__root.tsx` exports a `<RootRoute>` whose `component` is `<PortfShell>` — it wraps every child. Mounted once for the session; sidebar, theme wrapper, sonner toaster never tear down.
- `chat.$threadId.tsx`'s component is `<ChatPane>` which renders its own `<Outlet />`. Going from `/chat/$id` to `/chat/$id/artifact/$kind/$slug` adds a child route — TanStack Router renders the child into ChatPane's Outlet, ChatPane re-renders but does NOT unmount. The composer keeps typed-but-not-sent text.
- Split-view widthing is layout, not routing: `<ChatPane>` calls `useChildMatches()` (TanStack Router hook) — if any child match exists, it shrinks itself to 46% and lets the Outlet take 54%; otherwise full-width.
- Sidebar thread-switching (`/chat/A` → `/chat/B`) changes `$threadId`. React reconciler considers `<ChatPane>` the same instance (component identity) but its `useParams().threadId` returns the new id. To cleanly reset thread-local state (scroll position, in-flight SSE), the inner view is keyed: `<ThreadView key={threadId} threadId={threadId} />` — React unmounts the old ThreadView and mounts a fresh one, but the *shell + sidebar + composer chrome* stay live.
- Closing the artifact: `<ArtifactPane>`'s × calls `router.navigate({ to: '/chat/$threadId', params: { threadId } })` (typed). The outlet empties; `ChatPane` widens back. CSS `translateX(100%)` exit animation runs via `tailwindcss-animate` or hand-rolled keyframes — TanStack Router doesn't bundle a transition library, but it doesn't fight one either.

**Typed params + search:**

```ts
// routes/chat.$threadId.artifact.$kind.$slug.tsx
import { createFileRoute } from '@tanstack/react-router'

export const Route = createFileRoute('/chat/$threadId/artifact/$kind/$slug')({
  component: ArtifactPaneRoute,
  parseParams: ({ threadId, kind, slug }) => ({
    threadId,                               // string
    kind: kind as 'project' | 'writing' | 'cv',
    slug,
  }),
})

function ArtifactPaneRoute() {
  const { threadId, kind, slug } = Route.useParams()    // fully typed, no assertion
  // …
}

// routes/artifact.$kind.$slug.tsx — uses search-param validation for ?back=
import * as v from 'valibot'

const ArtifactSearchSchema = v.object({
  back: v.optional(v.pipe(v.string(), v.startsWith('/'))),   // must be a same-origin path
})

export const Route = createFileRoute('/artifact/$kind/$slug')({
  validateSearch: (search) => v.parse(ArtifactSearchSchema, search),
  component: ArtifactStandalone,
})
```

Invalid `?back=https://evil.com` URLs are rejected by `validateSearch` at the routing boundary — `ArtifactStandalone` never sees them. That's the kind of bug React Router would let through and we'd have to remember to guard at every read site.
...
- On `/chat/$id/artifact/$kind/$slug`, the artifact takes the full viewport (the chat is visually hidden via `hidden md:block` but stays mounted in the DOM). ← BACK navigates to `..` (`router.history.back()`).
- On `/artifact/$kind/$slug` (no thread), ← BACK navigates to `search.back ?? '/'`.
...
if (matched.kind === 'direct') {
  ...
  router.navigate({
    to: '/chat/$threadId/artifact/$kind/$slug',                      // typed route id
    params: { threadId, kind: matched.ref.kind, slug: matched.ref.slug },
  });
...
router.navigate({ to: '/chat/$threadId', params: { threadId } });    // no-op if already there
...
if (matched.kind === 'ambiguous') {
  threads.appendDisambig(threadId, matched.candidates);              // chip click → router.navigate({to: '/chat/$threadId/artifact/$kind/$slug', …})
}
```

Calling `router.navigate({ to: '/chat/$threadId/artifact/$kind/$slug', … })` from `/chat/$threadId` adds the nested segment — `ChatPane` stays mounted; its `<Outlet />` paints the artifact; the right pane animates in. No remount, no scroll loss, no composer-text loss. TanStack Router's typed `to:` value means a typo (`'/chat/$threadid/...'`) is a compile error, not a 404 at runtime.
...
| Phase | Title | Lands |
|---|---|---|
| A | Workspace plumbing + script rename + LTS dep audit | `.env.portf`, docker init, root scripts (incl. `dev` → `dev:simlm` rename + `dev:portf`), per-app `dev:portf` scripts, `provision:portf` one-shot, `turbo.json` task def, admin `VITE_ADMIN_API_TARGET` patch, **dep audit report** (see below) |
...
| C | `apps/portf` scaffold | Vite/Tailwind/React mirror of apps/web (NOT a pure copy: TanStack Router instead of react-router, `@tanstack/router-plugin` wired in vite.config.ts, `routeTree.gen.ts` git-ignored + tsconfig-included). `dev:portf` boots; blank `/` route renders Wordmark + press theme; all 25 themes shipped in `theme.css` |
...
**Phase A — LTS dep audit detail.** Per the user's standing rule (LTS-stable majors, caret ranges only — no hard pinning — and verify the chosen major is mainstream-stable before adopting): walk every `package.json` in root + `apps/*` + `packages/*`, query npm registry for `latest` and any explicit `lts` dist-tags, and produce `docs/dep-audit-2026-05-26.md` with a 3-column table per workspace: *current → latest LTS → action* (`keep`, `bump in this phase`, `bump in a follow-up phase`, `pin reason: <…>`). New deps that phase A *adds* (`@tanstack/react-router`, `@tanstack/router-plugin`, `@mdx-js/rollup`, `@mdx-js/react`, `gray-matter`, `uuid`) are caret-ranged to the latest LTS major at the time of writing. Existing-app bumps (e.g. Vite 6 → 7 if that's the LTS at audit time) require user sign-off before landing — the audit report is the gate. **Embargo discipline preserved**: `pnpm-workspace.yaml`'s `minimumReleaseAge: 10080` (7 days) refuses any package version published <1 week ago; bumps to bleeding-edge majors are automatically blocked.
...
- "Apps/portf uses TanStack Router (file-based) while apps/web uses react-router 7. Different routers per app is allowed — the apps are isolated. Don't 'unify' them."
- "TanStack Router's `routeTree.gen.ts` is git-ignored but tsconfig-included. Re-run `pnpm dev` (or build) to regenerate after adding/renaming a route file."
